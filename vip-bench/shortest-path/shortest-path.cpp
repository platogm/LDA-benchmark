...
//V represents the number of vertex in the graph
const int V = 10;
...
//This function finds the vertex with min distance from the unknown vertexes
void minVal(VIP_ENCINT dist[V],VIP_ENCBOOL known[V],VIP_ENCINT& distVal,VIP_ENCINT& min){
	min = -1;
	distVal = INT_MAX;
...
//Finds the shortest path from the source to all other vertexes
void shortestPath(VIP_ENCINT source,VIP_ENCINT graph[V][V],VIP_ENCINT path[V]){
	VIP_ENCINT dist[V];
	VIP_ENCBOOL known[V];
	VIP_ENCINT min = source;
	VIP_ENCINT distVal = 0;
		...
		for(int k = 0;k<V;k++){	
			for(int i=0;i<V;i++){
				for(int j=0;j<V;j++){
					...
					condition = (i==min) && !known[j] && (graph[i][j]!=0) && (graph[i][j]+distVal < dist[j]);
					dist[j]=VIP_CMOV(condition,graph[i][j]+distVal,dist[j]);
					path[j]=VIP_CMOV(condition,min,path[j]);
				}
			}
			minVal(dist,known,distVal,min);
		}
	#else		
		for(int i=0;i<V;i++){
			if(source == i){
		...
		for(int i=0;i<V;i++){
			for(int j = 0;j<V;j++){
				if(!known[j] && graph[min][j]!=0 && (graph[min][j]+distVal < dist[j])){
					dist[j] = graph[min][j]+distVal;
					path[j] = min;				
				}
			}
			minVal(dist,known,distVal,min);
...
int main(){
	...
	displayGraph(graph);
	{
		...
		shortestPath(source,graph,path);
	}
